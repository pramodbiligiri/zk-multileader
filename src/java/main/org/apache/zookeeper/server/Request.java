    ...
    public String getRootPath() {
		...
		case OpCode.create:
			...
			if(txn != null){
    			...
    			path = createTxn.getPath();
			}else if(request != null){
	            CreateRequest createRequest = new CreateRequest();   
	            try {
					ByteBufferInputStream.byteBuffer2Record(request, createRequest);
					request.rewind();				
			...
			return ZKPartitionUtils.getRootPath(path);
		...
		case OpCode.createSession:
			...
			if(txn != null){
				...
				bytePath= createTxn.getPath();
    			if(bytePath != null){
    				strPath = new String(bytePath);
    			}
			}else if(request != null){
				CreateSessionTxn createRequest = new CreateSessionTxn();   
	            try {
					ByteBufferInputStream.byteBuffer2Record(request, createRequest);
					request.rewind();				
					bytePath = createRequest.getPath();
					strPath = new String(bytePath);
				} catch (IOException e) {
					e.printStackTrace();
					return "";
			...
			return ZKPartitionUtils.getRootPath(strPath);
		}
		return "";
	}
}
