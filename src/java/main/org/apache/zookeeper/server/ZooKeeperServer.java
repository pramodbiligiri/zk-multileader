...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public void expire(Session session) {
    	//TODO: ZKP: Don't expire session, ever
...
//        close(sessionId);
    ...
    long createSession(ServerCnxn cnxn, byte passwd[], int timeout, String path) {
        ...
        byte[] pathBytes = null;
        if(path != null){
        	pathBytes = path.getBytes();
        	pathLength = pathBytes == null ? 0 : pathBytes.length;
    	}
        
		ByteBuffer to = ByteBuffer.allocate(4 + pathLength);
        to.putInt(timeout);
        if(path != null){
        	to.put(pathBytes);
        }
    ...
    public void processConnectRequest(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        if(LOG.isDebugEnabled()){
        	LOG.debug("Connection request details:" + connReq);
        	LOG.debug("Connection request timeout:" + sessionTimeout);
        }
        ...
        if (sessionTimeout < minSessionTimeout) {
        	...
        	if(LOG.isDebugEnabled()){
        		LOG.debug("Timeout is set to minimum:" + sessionTimeout);
        	}
        }
        ...
        if (sessionTimeout > maxSessionTimeout) {
        	...
        	if(LOG.isDebugEnabled()){
        		LOG.debug("Timeout is set to maximum:" + sessionTimeout);
        	}
        }
        
        //TODO: ZKP: Fix reading of sessionTimeout correctly
//        cnxn.setSessionTimeout(sessionTimeout);
        cnxn.setSessionTimeout(40000);
        ...
        if (sessionId != 0) {
            ...
            LOG.info("Client attempting to establish new session at "
                    + cnxn.getRemoteSocketAddress());
            byte[] reqPath = connReq.getPath();
            String path = reqPath == null ? null : new String(reqPath);
			createSession(cnxn, passwd, sessionTimeout, path);
        }
    }
